...
settingsRouter.patch('/', async (req, res) => {
  const userId = getUserId(req as Request & { userId?: string });
  const { highContrast, fontSizePercent, dyslexiaFont, reduceMotion, focusMode } = req.body;
  const db = await getDb();
  ...
  if (isAzureSql()) {
    if (existing) {
      await db.run('UPDATE user_settings SET high_contrast = ?, font_size_percent = ?, dyslexia_font = ?, reduce_motion = ?, focus_mode = ? WHERE user_id = ?', [hc, fs, df, rm, fm, userId]);
    } else {
      await db.run('INSERT INTO user_settings (user_id, high_contrast, font_size_percent, dyslexia_font, reduce_motion, focus_mode) VALUES (?, ?, ?, ?, ?, ?)', [userId, hc, fs, df, rm, fm]);
    }
  } else {
    await db.run(`
      INSERT INTO user_settings (user_id, high_contrast, font_size_percent, dyslexia_font, reduce_motion, focus_mode)
      VALUES (?, ?, ?, ?, ?, ?)
      ON CONFLICT(user_id) DO UPDATE SET high_contrast = excluded.high_contrast, font_size_percent = excluded.font_size_percent, dyslexia_font = excluded.dyslexia_font, reduce_motion = excluded.reduce_motion, focus_mode = excluded.focus_mode
    `, [userId, hc, fs, df, rm, fm]);
  }
  res.json({ highContrast: !!hc, fontSizePercent: fs, dyslexiaFont: !!df, reduceMotion: !!rm, focusMode: !!fm });
});
