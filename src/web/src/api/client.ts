...
export const api = {
  ...
  goals: {
    list: () => request<{ id: string; title: string; colorHex?: string | null; createdAt: string }[]>('/goals'),
  ...
  tasks: {
    list: (params?: { status?: string; type?: string }) => {
      ...
      return request<{ id: string; title: string; status: string; type: string; goalId?: string; dependencyTaskId?: string | null; dependencyStatus?: string | null; recurrenceRule?: 'daily' | 'weekly' | 'monthly' | null; plannedFor?: string | null; nextStep?: string; top3Candidate: boolean; top3Rank?: number | null; colorHex?: string | null; createdAt: string }[]>(`/tasks${query ? `?${query}` : ''}`);
    },
    top3: () => request<{ id: string; title: string; status: string; dependencyTaskId?: string | null; dependencyStatus?: string | null; recurrenceRule?: 'daily' | 'weekly' | 'monthly' | null; plannedFor?: string | null; nextStep?: string; timeboxMinutes?: number; top3Rank?: number | null; colorHex?: string | null; createdAt: string }[]>('/tasks/top3'),
    create: (data: { title: string; goalId?: string; type?: string; dependencyTaskId?: string | null; recurrenceRule?: 'daily' | 'weekly' | 'monthly' | null; plannedFor?: string | null; nextStep?: string }) => request('/tasks', { method: 'POST', body: JSON.stringify(data) }),
    update: (id: string, data: { title?: string; status?: string; goalId?: string | null; dependencyTaskId?: string | null; recurrenceRule?: 'daily' | 'weekly' | 'monthly' | null; plannedFor?: string | null; nextStep?: string; top3Candidate?: boolean; top3Rank?: number | null; colorHex?: string | null; pausedUntil?: string }) => request(`/tasks/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
