...
| **P-E7** Structured logging / dashboards | 🚧 In progress | First slice is built: structured JSON API logs, request IDs, route failure events, process-level logging, and an observability guide with starter Azure queries. Still needs dashboards and alert thresholds. |
